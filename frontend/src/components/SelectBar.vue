<template>
    <div class="selector">
        <span class="current" @click="toggle">
            {{ selectedText }}
            <button class="searcher" @click.stop="searching = !searching">🔍</button>
        ...
        <Teleport to="body">
            <div class="options teleportor" :class="{ opening }" :style="{
                ...
                <div>
                    <div class="option" v-if="options.length < 1">
                        {{ noOptionTip }}
                    </div>
                </div>
            </div>
...
async function everyFrame(executor: (stop: () => boolean) => void | Promise<void>) {
    async function loop() {
        let keep = true;
